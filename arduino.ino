...
const int stepsPerRevolutionunder = 1500;
...
void loop() { // run over and over
  ...
  if (Serial.available()) {
    ...
    if (myObject.hasOwnProperty("orders")) {
      ...
      int  type = myObject["type"];
        int x = myObject["x"];
        int y = myObject["y"];
        int dir = myObject["direction"];
      if (orders == 1) {
                if (type == 1) {
...
void FollowLine() {
  ...
  if ((LFSensor[0] == 0) && (LFSensor[1] == 1) && (LFSensor[2] == 1) && (LFSensor[3] == 1) && (LFSensor[4] == 1)) {
    Left(4);
...
void up(){
  ...
  // Spin motor quickly
  for(int x = 0; x < stepsPerRevolutionunder; x++)
  {
    digitalWrite(stepPin, HIGH);
    delayMicroseconds(2000);
    digitalWrite(stepPin, LOW);
